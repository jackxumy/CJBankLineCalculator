...
export function generatePerpendicularLines(
  ...
  for (let d = actualStart; d <= actualEnd; d += interval) {
    ...
    endpointData.push({ distance: d, left: leftCoords, right: rightCoords });

    perpendicularLines.push({
      type: 'Feature',
      properties: { distance: d, leftPoint: leftCoords, rightPoint: rightCoords },
      geometry: { type: 'LineString', coordinates: [leftCoords, rightCoords] }
    });
  }

  return { featureCollection: turf.featureCollection(perpendicularLines), endpointData };
}
